...
class ElDataset(Dataset):
    ...
    def __init__(self, df, samples, hist_hours=168, future_hours=24):
        """
        Args:
    ...
    def __getitem__(self, idx):
        ...
        return (torch.Tensor(self.raw_data.loc[hist_start:hist_end, household].values),
                torch.Tensor(self.raw_data.loc[future_start:future_end, household].values))
    ...
    def sample(self):
        """
        Create sampling. Note that we shuffle `idx`, otherwise we would yeild households in batches,
        ...
        idx = np.arange(self.samples * self.raw_data.shape[1])
        np.random.shuffle(idx)

        pairs = []

        for household in self.raw_data.columns:
            start_ts = timestamps.sample(self.samples)
