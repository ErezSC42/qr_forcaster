...
class DatasetHandler:
    ...
    def load_dataset(self, df: pd.DataFrame = None, dict_df_features:pd.DataFrame=None,split_train_val: bool = True, split_assets:bool=True, num_workers=0, test_size:float=0.5):
        if df is None:
            df, dict_df_features = self.load_df()
        if split_train_val:
            (df_target_train, dict_df_feature_train), (df_target_val, dict_df_feature_val) = self.split_df(df, dict_df_features, split_assets=split_assets, test_size=test_size)
            train_dataset = FinticaDataset(df=df_target_train, num_samples=self.num_samples, hist_days=self.hist_days,
                                      future_days=self.pred_horizon,
                                      forking_total_seq_length=self.forking_total_seq_length, dict_features =dict_df_feature_train )
            val_dataset = FinticaDataset(df=df_target_val, num_samples=self.num_samples, hist_days=self.hist_days,
        ...
        dataset = FinticaDataset(df=df, num_samples=self.num_samples, hist_days=self.hist_days,
                            future_days=self.pred_horizon,
                            forking_total_seq_length=self.forking_total_seq_length, dict_features=dict_df_features)
        dataloader = DataLoader(dataset, batch_size=self.batch_size, num_workers=num_workers)
        return dataloader, None

    def split_df(self, df:pd.DataFrame, dict_df_features:dict,split_assets: bool=True, test_size:float=0.5):
        num_rows, num_cols = df.shape
        train_size = int(num_rows * self.val_split_ratio)
        if split_assets:
            print("splitting assets!")
            assets = df.columns[1:]
            train_assets, test_assets = train_test_split(assets, test_size=test_size)
