    ...
    def get(self, asset, start_ts):
      #  print(colored(asset, "green"))
        if self.forking_total_seq_length is None:
            ...
            y_data = torch.Tensor(hist_slice[asset].values).unsqueeze(-1)
            
            # ----------ADDITIONAL FEATURES
            x_features_df = self.dict_features[asset].iloc[hist_start:hist_end] if self.dict_features[asset] is not None else None
